...
本文档面向后续参与 `neko_roast`（代号「猫娘锐评」，真身是**直播中心 / Live Center**）的开发者，记录**已落地设计**。配套 `live-center-roadmap.md`（定位 / 进度 / 路线 / 下一阶段 TODO）。
...
> 注：`plugin/tests/unit/server/test_plugin_ui_query_service.py` 是 host 侧测试（且属 roadmap §9「不要碰的在途 WIP」），不在 neko_roast 验证范围内。

## 新功能文档要求

后续新增功能模块时，开发者必须同步留下对应文档。至少包含：
